...
/**
 * Swiss-style minimal legend for the Poincare disk graph.
 * Shows node type indicators and relationship type labels.
 */
export function GraphLegend({ className = "", compact = false }: GraphLegendProps) {
    ...
    const relationships = Object.entries(RELATIONSHIP_LABELS).slice(0, 4).map(([type, label]) => ({
        type,
        label,
    }));
    ...
    if (compact) {
        return (
            <div className={`flex items-center gap-3 text-[10px] text-white/60 ${className}`}>
    ...
    return (
        <div className={`bg-black/40 backdrop-blur-md rounded-lg p-3 border border-white/10 ${className}`}>
            {/* Node Types */}
            <div className="mb-3">
